...
class Sections(BaseModel):
    sections: List[Section] = Field(
        ...,
        title="Sections",
        description="The segmented sections of the call transcript.",
    )
...
agent_templates = {
    ...
    SECTION_MERGER: {
        "GeneralizedSchemaGenerator": dict(
...
async def extract_call_structure(
    transcripts: List[str], llm: LLM, inputs_dict, group_size=4
):
    crew = get_section_crew(SECTION_EXTRACTION, llm)
    responses = list()
    for i in tqdm(
        range(0, len(transcripts), group_size), desc="Extracting Call Structure"
    ):
...
async def merge_call_structures(
    sections: List[CrewOutput], llm: LLM, inputs_dict, group_size=4
) -> List[CrewOutput]:
    if len(sections) == 1:
        return sections
...
async def aget_call_structure(transcripts, llm, inputs_dict: Dict) -> CrewOutput:
    ...
    call_structure_responses: List[CrewOutput] = await extract_call_structure(
        transcripts, llm, inputs_dict, group_size=4
    )
    call_structure = await merge_call_structures(
        call_structure_responses, llm, inputs_dict, group_size=4
    )
