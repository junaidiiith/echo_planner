    ...
    for initiative in title_data["Strategic Initiatives"]:
        ...
        for tag in keys:
            ...
            for teams in initiative[tag]:
                titles_data[teams["team"]] = {
                    "Relevance": teams["Relevance"],
                    "titles": teams["titles"],
                }
                all_titles_to_search = all_titles_to_search + teams["titles"]
                final_strategy_data[initiative["Buyer Initiative or pain"]][tag].append(
                    titles_data[teams["team"]] | {"Team": teams["team"]}
                )
...
def generate_value_props_for_stakeholders(
    graph: "StakeholderGraph", top_targets: dict, initiative, buyer, seller
):
    value_props = []
    email_snippets = []
    ...
    for tag, stakeholders in top_targets.items():
        ...
        """
              ...
              """,
                sub_queries=[
                    LlamaSubQuery(
                        ...
                        inputs={
                            "category": BuyerIndexQueryTypes.STRATEGIC_INITIATIVES.value
                        },
                    ),
                    LlamaSubQuery(
                        ...
                        index_type=IndexType.BUYER_WEB_SEARCH,
                        inputs={
                            "query_type": BuyerIndexQueryTypes.LEADERSHIP_MOVEMENTS.value
                        },
                    ),
                    LlamaSubQuery(
            ...
            person_role_response = asyncio.run(
                arun_query_chain(
                    query_chain=QueryChain(queries=[value_prop_alignment]),
                    inputs=inputs,
                )
            )
            ...
            one_liner = re.search(
                r"\*\*1-liner Email Summary:\*\* (.+)",
                person_role_response.responses[0].response,
            )
            email_snippets.append(
                f"- {full_name} ({tag}): {one_liner.group(1) if one_liner else '...'}"
            )
...
print(response['choices'][0]['message']['content'])
  ...
  """
    ...
    try:
        person_data_relevant["role_enriched"] = json.loads(
            ...
            for team in strategic_data[strategy][tag]:
                ...
                for title in titles_needed:
                    for people in people_data["profiles"]:
                        if semantic_match(
                            model, title, people["default_position_title"], 0.70
                        ) or (
                            (
                                float(
                                    rapidfuzz.fuzz.token_set_ratio(
                                        title, people["default_position_title"]
                                    )
                                )
                                / 100.0
                            )
                            ...
                            if people["name"] not in person_names:
                                ...
                                persons.append(
                                    extract_relevant_person_data(people, buyer)
                                )
                team |= {"people": persons}
...
def join_people_strategy_data(buyer, people_data, final_strategy_data):
    ...
    people_strategy_data = match_people_strategy_data(
        people_data, final_strategy_data, buyer
    )
    # print(result)
                ...
                for person in team["people"]:
                    ...
                    """
                            ...
                            node_data = json.loads(
                                node_influence.output_text.split("```json")[1]
                                .strip("`")
                                .strip()
                            )
...
class StakeholderGraph:
    ...
    def _calculate_edge_weight(self, a, b):
        ...
        tags_weights = {
            ...
            "Economic Buyer": 5,
            "Direct Owner Team": 3,
            "Cross-Functional Reviewers": 1,
            "Internal Influencers": 2,
        }
...
def remove_duplicates(people: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    all_people_names = {}
    all_people_deduped = []
...
def fetch_all_profiles(header, buyer, all_titles_to_search, max_pages=4, delay=1.0):
    ...
    while True:
        ...
        response = requests.post(
            ...
            json={
                "filters": [
                    {"filter_type": "CURRENT_COMPANY", "type": "in", "value": [buyer]},
                    {
...
def get_graphs_for_initiatives(strategy_people_data_scored: Dict[str, Dict[str, List[Dict[str, Any]]]]):
    ...
    for initiative in a:
        all_people[initiative] = remove_duplicates(
            copy.deepcopy(all_people[initiative])
        )
